...
class GoogleSheet{
    ...
    public function __construct()
    {
        ...
        $this->client->setAuthConfig(storage_path('reportesmoodle-e8d72157f72f.json'));
    ...
    public function readGoogleSheet()
    {
        ...
        // dd($dimensions);
        $range = 'Reporte!A1:' . $dimensions['colCount'];
    ...
    public function saveDataToSheet(array $data,$idSheet,$nameSheet)
    {
        $dimensions = $this->getDimensions($idSheet,$nameSheet);
        ...
        return $this->googleSheetService
            ->spreadsheets_values
            ->update($idSheet, $range, $body, $params);
    }
    ...
    private function getDimensions($spreadSheetId)
    {
        $rowDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'Reporte!A:A', 'majorDimension' => 'COLUMNS']
        );
        ...
        $colDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'Reporte!1:1', 'majorDimension' => 'ROWS']
        );
